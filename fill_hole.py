...
def empty_fill_only0():
    print('empty_fill..............')
    data_path = 'result'
    fill_path = 'submit'
    predict_3 = os.path.join(data_path, 'image_5_predict.png')
    predict_4 = os.path.join(data_path, 'image_6_predict.png')
    predict_3_ = os.path.join(fill_path, 'image_5_predict.png')
    predict_4_ = os.path.join(fill_path, 'image_6_predict.png')
    predict = [predict_3, predict_4]
    predict_ = [predict_3_, predict_4_]

    for i in range(len(predict)):
        print('fill {} predict img'.format(i + 3))
        img = Image.open(predict[i]).convert('L')
        img = np.asarray(img)
        img_ = img.copy()
        ...
        for x in range(0, height, length // temp):
            ...
            for y in range(0, weight, length // temp):
                ...
                if len(set(x1)) == len(set(x2)) == len(set(y1)) == len(set(y2)) == 1 and x1[0] == x2[0] == y1[0] == y2[0]:
                    squere = img_[x:x + length, y:y + length]
                    img_[x:x + length, y:y + length][squere == 0] = x1[0] # only fill label=0
        ...
        #         y2 = list(y2)
        #         if len(set(x1)) == len(set(x2)) == len(set(y1)) == len(set(y2)) == 1 and x1[0] == x2[0] == y1[0] == y2[0]:
        #             img_[x:x + length, y:y + length][img_[x:x + length, y:y + length] == 0] = x1[0]
...
def labelVis():
    ...
    predict_3_ = os.path.join(fill_path, 'image_5_predict.png')
    predict_4_ = os.path.join(fill_path, 'image_6_predict.png')
    vis_img3 = os.path.join(fill_path, 'fill_vis_5.png')
    vis_img4 = os.path.join(fill_path, 'fill_vis_6.png')
    predict_ = [predict_3_, predict_4_]
...
def static():
    ...
    for i in range(len(predict)):
        ...
        img = Image.open(predict[i]).convert('L')
        img = np.asarray(img)
        img_ = Image.open(predict_[i]).convert('L')
