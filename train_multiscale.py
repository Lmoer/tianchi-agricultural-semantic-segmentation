...
from crops_dataset_448 import CropSegmentation1
...
def main():
    ...
    model_fname = '../data/448_512_16(1)/deeplabv3_{0}_epoch%d.pth'.format('crops')
    
    train_dataset = CropSegmentation(train=True, crop_size=args.crop_size)
    train_dataset1 = CropSegmentation(train=True, crop_size=448)
...
#         scheduler = lr_scheduler.CosineAnnealingLR(optimizer,T_max = (args.epochs // 9) + 1)
        ...
        dataset_loader1 = torch.utils.data.DataLoader(
                        train_dataset1, batch_size=20, shuffle=args.train,
                        pin_memory=True, num_workers=args.workers)
...
#             scheduler.step(epoch)
            model.train()
            for i, (inputs, target) in enumerate(dataset_loader1):

                inputs = Variable(inputs.cuda())
                target = Variable(target.cuda())
                outputs = model(inputs)
                loss4 = focal_loss(outputs['out'], target)
                loss5 = focal_loss(outputs['aux'], target)
                loss6 = loss4 + 0.2*loss5
                loss7 = lovasz_softmax(outputs['out'], target)
                loss8 = lovasz_softmax(outputs['aux'], target)
                loss9 = loss7 + 0.2*loss8
                loss = loss6 + loss9
                ...
                print('epoch: {0}\t'
                      ...
                      epoch + 1, i + 1, len(dataset_loader1),loss=losses))
